...
class ParkingOperations {
  static create()  {
   ...
   ('Enter adress');
    ...
    if (Validator.isString(regnr)&& Validator.isString(adress)) {
      Parking parking = Parking(regnr: regnr!, id:Uuid().v4(),adress: adress!);
      repository.create(parking);
  ...
  static Future update()async {
    ...
    if (Validator.isIndex(input, allParking)) {
      ...
      if (Validator.isString(regnr)&& Validator.isString(adress)) {
        Parking newparking = Parking(regnr: regnr!, id: parking.id, adress: adress!);
        repository.update(newparking);
  ...
  static Future delete()async {
    ...
    if (Validator.isIndex(input, allParking)) {
      int index = int.parse(input!) - 1;
      repository.delete(allParking[index].id);
